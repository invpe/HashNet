...
void CServer::Tick()
{
                    ...
                    if (err.empty()) {
                        ...
                        if (jsondata.is_object() && jsondata["params"].is_array()) {
                            ...
                            if(m_tCurrentJob.m_mMapping["clean_jobs"]=="true")
                            {
                                ...
                                SaveLog(SERVER_LOG_INFO, "Clean jobs flag detected. Resetting nonce chunks, best_extranonce2, and disconnecting busy clients.");
                                DisconnectAll();

                                uint8_t target[32];
                                NbitsToTarget(m_tCurrentJob.m_mMapping["nbits"],target);
    ...
    for(size_t x = 0; x < m_vpSessions.size(); x++)
    {
        ...
        for (const auto& command : vCommands) 
        {
            ...
            if (method == "found") { 
                ...
                SaveLog(SERVER_LOG_WARRNING, pool_response);
                ...
                SaveLog(SERVER_LOG_INFO, "Job: "+m_tCurrentJob.m_mMapping["job_id"]+" "+pClient->GetIdent()+" DONE "+pClient->GetExtranonce2()+" <"+std::to_string(pClient->GetStartNonce())+","+std::to_string(pClient->GetEndNonce())+">");
        ...
        {
            // Obtain work chunk from current task
            std::string strExtranonce2 = m_tCurrentJob.GetNextExtranonce2();  
            std::pair<uint32_t, uint32_t> ppWorkChunk = m_tCurrentJob.GetNextWorkChunk(strExtranonce2);
            ...
            SaveLog(SERVER_LOG_INFO, "Job: "+m_tCurrentJob.m_mMapping["job_id"]+" "+pClient->GetIdent()+" SENT "+pClient->GetExtranonce2()+" <"+std::to_string(pClient->GetStartNonce())+","+std::to_string(pClient->GetEndNonce())+">");
        }
    ...
    {   
        ...
        SaveLog(SERVER_LOG_INFO, "Job: "+m_tCurrentJob.m_mMapping["job_id"]+" Miners: "+std::to_string(m_vpSessions.size())+" C/S: "+std::to_string(m_tStatistics.m_uiCombinationsPerSecond)+" Top: "+std::to_string(m_tCurrentJob.m_mBestExtranonce2.size())+" Dst: "+std::to_string(m_tCurrentJob.GetBestDistance()));
        ...
        if (!m_vpSessions.empty()) {
            ...
            for (auto it = m_vpSessions.begin(); it != m_vpSessions.end(); ++it) {
                ...
                strJSON += "\"promising\": "+std::to_string(pClient->GetPromisingShares())+"\n";
...
void CServer::NbitsToTarget(const std::string &nbits, uint8_t target[32]) {
